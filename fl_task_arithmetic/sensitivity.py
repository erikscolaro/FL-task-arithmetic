...
def compute_sensitivity_scores(
    ...
    params_fixed.update(
        {k: v for k, v in model.named_parameters() if not v.requires_grad}
    )
    buffers = {k: v for k, v in model.named_buffers()}

    sensitivity_scores = {
        k: torch.zeros_like(v, device="cpu") for k, v in params_diff.items()
    }
            ...
            for i in range(0, total_samples, MICRO_BATCH_SIZE):
                ...
                for name, g_sample in chunk_grads.items():
                    ...
                    sensitivity_scores[name] += (
                        g_sample.pow(2).sum(dim=0).detach().cpu()
                    )
...
def calibrate_gradient_masks(
    ...
    for round_idx in range(num_calibration_rounds):
        ...
        all_scores = torch.cat(
            [sensitivity_scores[name].flatten() for name in sensitivity_scores]
        )
        ...
        print(
            f"  Non-zero scores: min={all_scores[all_scores > 0].min():.2e}, "
        ...
        print(
            f"  Result: {frozen_params}/{total_params} frozen ({actual_sparsity:.2%})"
        )
    ...
    print(
        f"Final: {final_frozen}/{final_total} parameters frozen ({final_frozen/final_total:.2%})"
    )
    if final_frozen == 0:
        print(
...
def calibrate_gradient_masks_with_lowest_magnitudes(
    ...
    all_magnitudes = torch.cat(
        [param.detach().abs().reshape(-1).cpu() for _, param in param_infos]
    )
